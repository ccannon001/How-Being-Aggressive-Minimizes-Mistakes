...
ep_per_event_table <- pbp %>%
  ...
  mutate(
    ...
    epa_per_sack_interception_rate = round(epa_per_sack_interception_rate, 3),
  ) %>%
  arrange(season)
...
ep_per_event_table %>%
  gt() %>%
  tab_header(
    title = "EPA per Event: Interceptions vs Non Fumble Sacks",
  ...
  cols_label(
    ...
    sacks_no_fumble = " Total Sacks",
    epa_per_sack_interception_rate = "Conv EPA/Sack",
